...
function suggestAlts(filename: string, w?: number, h?: number): string[] {
  ...
  if (base && dims) list.add(`${base} (${dims})`);
  if (dims) list.add(`Image ${dims}`);
  list.add("Illustrative image");
...
export default function Page() {
  ...
  const altCandidates = useMemo(
    () => suggestAlts(file?.name || "", dims?.w, dims?.h),
    [file?.name, dims?.w, dims?.h]
  );
  ...
  async function convert() {
    ...
    try {
      ...
      const url = canvas.toDataURL(format, format === "image/jpeg" ? quality : undefined);
      setDataUrl(url);
    } finally {
      setBusy(false);
    }
