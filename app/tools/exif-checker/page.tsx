...
// Minimal EXIF detector + GPS tag (0x8825) finder for JPEG APP1/Exif
        ...
        if (byteOrder === 0x4d4d || byteOrder === 0x4949) {
          ...
          if (fortyTwo === 0x2a) {
            ...
            if (ifdOffset + 2 <= view.byteLength) {
              ...
              // Next IFD offset is at end of IFD0 (after tags * 12 + 2)
              const nextIfdOffsetPos = ifdOffset + 2 + numTags * 12;
              let exifIfdOffset = 0;
              for (let t = 0; t < numTags; t++) {
                ...
                if (tag === 0x8769) {
                  // ExifIFDPointer
                  exifIfdOffset = getUint32(view, base + 8, little);
                }
              }
              // GPS IFD pointer is a tag in IFD0: 0x8825
              let hasGps = false;
              for (let t = 0; t < numTags; t++) {
                const base = ifdOffset + 2 + t * 12;
                const tag = getUint16(view, base, little);
...
export default function Page() {
  ...
  async function onStrip() {
    ...
    setBusy(true);
    try {
      const clean = await removeMetadata(inputRef.current.files[0]);
