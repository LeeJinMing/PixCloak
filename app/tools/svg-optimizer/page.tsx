...
function optimizeSvg(input: string): { out: string; saved: number } {
  // very small local optimizer: strip comments, collapse whitespace, remove metadata tags
  ...
  let out = input
    ...
    .replace(/<\?xml[^>]*>/g, "")
    .replace(/<!DOCTYPE[^>]*>/g, "")
    .replace(/\s+xmlns(:[a-zA-Z0-9-]+)?="[^"]*"/g, (m) => m) // keep xmlns
    .replace(/\s+xmlns:xlink="http:\/\/www\.w3\.org\/1999\/xlink"/g, "") // remove xlink xmlns if unused
