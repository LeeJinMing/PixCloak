...
export default function Page(){
  ...
  async function onGenerate(){
    ...
    items.forEach((it, idx)=>{
      const r = Math.floor(idx / colCount); const cidx = idx % colCount; const x = cidx * (cellW + gap); const y = r * (cellH + gap);
    ...
    if(!png) return;
    const zip = new JSZip(); zip.file('sprite.png', png); zip.file('sprite.json', JSON.stringify({ cell: { w: cellW, h: cellH, gap }, mapping }, null, 2));
    const content = await zip.generateAsync({ type:'blob' });
    const url = URL.createObjectURL(content); const a=document.createElement('a'); a.href=url; a.download='sprite-sheet.zip'; a.click(); URL.revokeObjectURL(url);
  }
