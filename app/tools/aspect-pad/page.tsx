...
export default function Page(){
  ...
  async function onPad(){
    ...
    if(src > tgt){ // too wide -> pad height
      H = Math.round(img.width / tgt); y = Math.round((H - img.height)/2);
      cW = img.width; cH = img.height;
    } else if(src < tgt){ // too tall -> pad width
      W = Math.round(img.height * tgt); x = Math.round((W - img.width)/2);
      cW = img.width; cH = img.height;
    }
