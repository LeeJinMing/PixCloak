...
export default function Page(){
  ...
  const [tol, setTol] = useState(900); // ~30 per channel squared
  const [outUrl, setOutUrl] = useState<string | null>(null);

  async function onPick(e: React.ChangeEvent<HTMLInputElement>){
    const f = e.target.files?.[0]; if(!f) return; const url = URL.createObjectURL(f); setPreview(url); setOutUrl(null);
  }
