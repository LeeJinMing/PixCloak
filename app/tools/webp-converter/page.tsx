  ...
  async function onConvertZip() {
    ...
    try {
      ...
      for (const f of files) {
        const b = await convertToWebp(f, quality);
        const base = f.name.replace(/\.[^.]+$/, '');
        zip.file(`${base}.webp`, b); idx++;
      }
      const content = await zip.generateAsync({ type: 'blob' });
      const url = URL.createObjectURL(content); const a = document.createElement('a'); a.href = url; a.download = 'webp.zip'; a.click(); URL.revokeObjectURL(url);
